 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */class eu{constructor(e){this.apiKey=e}getGenerativeModel(e,t){if(!e.model)throw new L("Must provide a model name. Example: genai.getGenerativeModel({ model: 'my-model-name' })");return new ed(this.apiKey,e,t)}getGenerativeModelFromCachedContent(e,t,n){if(!e.name)throw new k("Cached content must contain a `name` field.");if(!e.model)throw new k("Cached content must contain a `model` field.");for(let n of["model","systemInstruction"])if((null==t?void 0:t[n])&&e[n]&&(null==t?void 0:t[n])!==e[n]){if("model"===n&&(t.model.startsWith("models/")?t.model.replace("models/",""):t.model)===(e.model.startsWith("models/")?e.model.replace("models/",""):e.model))continue;throw new k(`Different value for "${n}" specified in modelParams (${t[n]}) and cachedContent (${e[n]})`)}let s=Object.assign(Object.assign({},t),{model:e.model,tools:e.tools,toolConfig:e.toolConfig,systemInstruction:e.systemInstruction,cachedContent:e});return new ed(this.apiKey,s,n)}}const eh=document.getElementById("taskInput");document.getElementById("taskInput2");const ef=document.getElementById("taskInput3"),eg=document.getElementById("addTaskBtn"),eE=document.getElementById("taskList"),em=document.getElementById("send-btn"),ep=document.getElementById("chat-input"),eC=document.getElementById("chat-history"),ey=document.getElementById("signOutBttn"),eO=JSON.parse(localStorage.getItem("email"));async function eI(){T=new eu((await (0,b.getDoc)((0,b.doc)(S.db,"apikey","googlegenai"))).data().key).getGenerativeModel({model:"gemini-1.5-flash"})}function ev(e){let t=document.createElement("div");t.textContent=e,t.className="history",eC.appendChild(t),ep.value=""}async function e_(e){ev((await T.generateContent(e)).response.text())}async function eT(e){let t=await eA(e);eh.value="",eb(t,e)}async function eN(e){await (0,b.updateDoc)((0,b.doc)(S.db,"scheduler",e),{completed:!0})}function ew(e){document.getElementById(e).remove()}async function eR(){var e=await eS();eE.innerHTML="";let t=[];e.forEach(e=>{t.push({id:e.id,text:e.data().text,completed:e.data().completed})}),t.sort(function(e,t){return new Date(t.timeCreated)-new Date(e.timeCreated)}),t.forEach(e=>{e.completed||eb(e.id,e.text)})}async function eA(e){return(await (0,b.addDoc)((0,b.collection)(S.db,"scheduler"),{text:e,email:eO,completed:!1,genre:ef})).id}async function eS(){let e=(0,b.query)((0,b.collection)(S.db,"scheduler"),(0,b.where)("email","==",eO));return await (0,b.getDocs)(e)}function eb(e,t){let n=document.createElement("li");n.id=e,n.textContent=t,n.tabIndex=0,n.setAttribute("name",t.toLowerCase()),eE.appendChild(n)}eO||(window.location.href="index.html"),window.addEventListener("load",async()=>{eI(),eR()}),em.addEventListener("click",async()=>{let e=ep.value.trim().toLowerCase();e?!function(e){if(e.startsWith("add book")){let t=e.replace("add book","").trim();return t?(eT(t),ev("Book "+t+" added!")):ev("Please specify a book to add."),!0}if(e.startsWith("remove")){var t;let n,s=e.replace("remove","").trim();return s?(t=s,0!=(n=document.getElementsByName(t)).length&&(n.forEach(e=>{eN(e.id),ew(e.id)}),1))?ev("Book "+s+" has been removed."):ev("Book not found!"):ev("Please specify a book to remove."),!0}return!1}(e)&&e_(e):ev("Please enter a prompt")}),ep.addEventListener("keypress",function(e){"Enter"===e.key&&em.click()}),eg.addEventListener("click",async()=>{let e=eh.value.trim();e?await eT(e):alert("Please enter a book!")}),eE.addEventListener("click",async e=>{"LI"===e.target.tagName&&(eN(e.target.id),ew(e.target.id))}),eh.addEventListener("keypress",function(e){"Enter"===e.key&&eg.click()}),eE.addEventListener("keypress",async function(e){"LI"===e.target.tagName&&"Enter"===e.key&&(eN(e.target.id),ew(e.target.id))}),window.addEventListener("error",function(e){console.error("Error occurred: ",e.message)}),ey.addEventListener("click",function(e){localStorage.removeItem("email"),window.location.href="index.html"});
//# sourceMappingURL=tasks.735edd10.js.map
